...
export default function CourseCard({ course }: CourseCardProps) {
  ...
  const handleDelete = () => {
    if (course.students > 0) {
      alert('Không thể xoá khoá học đang có học viên.');
      return;
    }
  ...
  return (
    <>
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, ml: 2 }}>
          ...
          <IconButton
            ...
            title={course.students > 0 ? 'Không thể xoá khoá học đang có học viên' : 'Xóa'}
          >
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        ...
        <DialogContent>
          <DialogContentText>
            Bạn có chắc chắn muốn xoá khoá học &quot;<strong>{course.title}</strong>&quot; không? Hành động này không
            thể hoàn tác.
          </DialogContentText>
        </DialogContent>

        <DialogActions>
          <Button onClick={() => setOpenDeleteDialog(false)}>Huỷ</Button>
